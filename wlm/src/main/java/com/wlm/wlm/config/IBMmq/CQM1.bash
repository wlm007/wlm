...
#启动队列管理器
strmqm $QmgrName
#定义一个服务器连接通道（供管理工具与代码调用使用，此处可去掉）
...
#设置该队列管理器为集群（MYClUSTER）的完全仓储库
echo "ALTER QMGR REPOS(MYClUSTER)" | runmqsc $QmgrName
#定义集群（MYClUSTER）接收通道
echo "DEFINE CHANNEL(TO.CQM1) CHLTYPE(CLUSRCVR) TRPTYPE(TCP) CONNAME('192.168.137.11(1415)') CLUSTER(MYClUSTER)" | runmqsc $QmgrName
#定义集群（MYClUSTER）发送通道
echo "DEFINE CHANNEL(TO.CQM2) CHLTYPE(CLUSSDR) TRPTYPE(TCP) CONNAME('192.168.137.12(1415)') CLUSTER(MYClUSTER)" | runmqsc $QmgrName
