...
public class WxServiceImpl {
     ...
     * 获取公众号的自动回复规则
     */
    private String getWxReplyRulesUrl = "https://api.weixin.qq.com/cgi-bin/get_current_autoreply_info?access_token=ACCESS_TOKEN";
    ...
    @Autowired
    public void setWxTokenMapper(WxTokenMapper wxTokenMapper) {
    ...
    public void handleWxMsg(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (WxMsgTypeEnums.WX_MSG_TYPE_EVENT.getMsgType().equals(msgType)) {
            ...
            if (WxEventEnums.WX_EVENT_TYPE_CLICK.getEventType().equals(eventType)) {
                ...
                this.setMsgData(receiverMsg, textMessage, WxMsgTypeEnums.WX_MSG_TYPE_TEXT.getMsgType());
                resStr = MessageUtils.textMessageToXml(textMessage);
            } else if (WxEventEnums.WX_EVENT_TYPE_SCAN.getEventType().equals(eventType)) {
                // 扫码事件
                TextMessage textMessage = new TextMessage();
                textMessage.setContent("煞笔");
                this.setMsgData(receiverMsg, textMessage, WxMsgTypeEnums.WX_MSG_TYPE_TEXT.getMsgType());
                resStr = MessageUtils.textMessageToXml(textMessage);
            }
        } else {
    ...
    public JSONObject getWxReplyRules() {
        return httpsRequest(getWxReplyRulesUrl.replace(ACCESS_TOKEN, token), GET, null);
    }
}
